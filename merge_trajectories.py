#use python merge_trajectories.py -v ./a1.webm -f ./out_of_tracks_debug.txt -s 3 -t 2
...
while(True):
    ...
    for p in numbers:
        if(p[0]-1 == i):
            cv2.circle(frame, (p[2*(l+1)-1],p[2*(l+1)]), 1, (0,255,0))

        if(p[0]-1 == i - l + 1):
            cv2.circle(frame, (p[2*(l+1)-1],p[2*(l+1)]), 1, (255,0,0))
